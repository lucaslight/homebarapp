...
def cocktails_dataset
  ...
  # cocktails = RestClient.get("www.thecocktaildb.com/api/json/v2/#{api_key}/search.php?s=bacardi")

  # cocktails_array << JSON.parse(cocktails)["drinks"]
  cocktails_array = cocktails_array.flatten


  # p cocktails_array.last['strDrink']
  # p "************************"
  # p cocktails_array.last["strDrink"]
  ...
  cocktails_array.each do |c|
    var = Cocktail.create!(
      name: c['strDrink'], #TypeError: no implicit conversion of String into Integer !!! :(
  ...
  puts "************ Creating measurements and ingredients *************"
    ...
    cocktails_array.each do |c|
      ...
      ingredients_api[c['strDrink']] = [ingredient1, ingredient2, ingredient3, ingredient4, ingredient5, ingredient6, ingredient7,
                                        ingredient8, ingredient9, ingredient10, ingredient11, ingredient12, ingredient13,
                                        ingredient14, ingredient15].reject { |ing| ing == nil }

      uniq_ingredients_api = ingredients_api.values.flatten.uniq
      ...
      measurements_api[c['strDrink']] = [measure1, measure2, measure3, measure4, measure5, measure6, measure7,
        measure8, measure9, measure10, measure11, measure12, measure13,
        measure14, measure15].reject { |m| m == nil }

      # p measurements_api[c['strDrink']]
      # p ingredients_api[c['strDrink']]


      measurements_api[c['strDrink']].each_with_index do |measure, i|
        cocktail_found = Cocktail.find_by(name: c['strDrink'])
        ingredient = Ingredient.find_by(name: ingredients_api[c['strDrink']][i])
